...
int main(){
    ...
    cout << "¿Va a ingresar nuevas ciudades? 1:si 0:no" << endl;
    bool opt = false;
    
    cin >> opt;
    int ciu;
    if(opt){
        cout << "¿Cuantas ciudades quiere ingresar?" << endl;
    ...
    for(int i=0; i<ciu; ++i){
    if(opt){
            cout << "Ingrese " << N << " distancias para dicha ciudad hacia otras ciudades en orden." << endl;

            vector<double> pesosto;
            vector<double> pesosfrom;
            for(int i=0; i<N; ++i){
                double aux;
                cin >> aux;
                pesosto.push_back(aux);
            }
            pesosto.push_back(0);
            cout << "Ingrese " << N << " distancias para dicha ciudad desde otras ciudades en orden." << endl;
            for(int i=0; i<N; ++i){
                double aux;
                cin >> aux;
                pesosfrom.push_back(aux);
            }
            

            GRAFITO.push_back(pesosto);
            for(int i=0; i<N; ++i){
                GRAFITO[i].push_back(pesosfrom[i]);
            }
            N++;
